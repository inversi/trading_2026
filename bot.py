...
#   ATR_K                Множитель ATR для начального стоп-лосса
...
#   MAX_LOSSES_IN_ROW    Лимит серии убыточных сделок подряд
...
class BreakoutWithATRAndRSI:
    ...
    Стратегия (лонг-только, учебный пример):
      Вход (на 1m):
        - Закрытие > SMA20 И пробой максимума предыдущей свечи
        - ATR% в диапазоне [ATR_PCT_MIN, ATR_PCT_MAX]
        - Если FAST_MODE включён: на HTF (например 5m) RSI >= FAST_RSI_MIN
          и текущий объём на 1m >= SMA(объёма, FAST_MIN_VOL_SMA)

      Риск и выход:
        - Размер позиции может рассчитываться либо от доли депозита, либо, как в текущей конфигурации,
          от фиксированной суммы входа (≈10 EUR на сделку).
        - В данной конфигурации стоп-лосс на бирже не ставится: выход происходит по тейк-профиту (TP)
          или вручную.
        - TP по умолчанию рассчитывается как entry + TP_R_MULT * (entry - виртуальный стоп),
          где виртуальный стоп = entry - ATR_K * ATR (используется только для расчёта расстояния до TP).

      ВАЖНО: код приведён в образовательных целях. Перед реальной торговлей —
      тестируйте в paper, проверяйте логи/ордера/точности и учитывайте комиссии/скольжение.
    """
    ...
    def _place_orders(self, symbol: str, qty: float, entry: float, atr_val: float) -> Optional[Position]:
        # Выставление ордеров:
        #  - в paper-режиме — просто создаём виртуальную позицию
        #  - в live — рыночная покупка + одиночный лимитный TP (без стоп-лосса)
        #
        # Стоп-лосс здесь считается только виртуально (для расчёта расстояния до TP),
        # но на бирже не выставляется.
